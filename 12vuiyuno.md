# 第12節:谷山–志村–Weil定理（半安定の場合）

**谷山–志村–Weil予想**（現在は完全に証明され、**モジュラー定理**と呼ばれる）は、一見すると大きく異なる 2 つの分野が深く結びついていることを示すものでした。
...
もともと算術幾何（楕円曲線側）と複素解析（モジュラー形式側）という別々の分野で研究されていた両者ですが、「$$\mathbb{Q}$$ 上のすべての楕円曲線は重み 2 のあるモジュラー形式と対応づけられる」という驚くべき予想（1950 年代の谷山豊の予想を、志村五郎とアンドレ・Weilが精緻化）によって、この 2 つの対象が深いレベルで同一視できるのではないかと考えられるようになりました。1990 年代、アンドリュー・ワイルズの画期的な研究によって、この対応が**半安定**楕円曲線について証明されました。その後、Breuil・Conrad・Diamond・Taylor らの仕事によって、すべての $$\mathbb{Q}$$ 上の楕円曲線に対して定理が拡張され、完全な形での「谷山–志村–Weil定理」が確立されました。

この節では、以下の内容を扱います。

* **半安定**な場合における谷山–志村–Weil定理（楕円曲線の特定の還約性質に関するモジュラー性）
...
歴史的には、ワイルズが最初に証明した谷山–志村–Weil定理はこの「半安定」な場合でした。その後、半安定性の仮定は除かれ、より一般の場合へ拡張されました。
...
> **重要な事実**: 重み 2 の場合、多くの幾何的・算術的性質（Galois 表現や $$L$$-級数など）が楕円曲線と直接対応します。そのため、谷山–志村–Weil定理では重み 2 に注目します。

***

### 12.1.2 半安定な谷山–志村–Weil定理の主張
...
実際的には、強い版が谷山–志村–Weilの**完全な**主張を与えますが、弱い版でも多くの古典的な問題を解決するには十分でした。

***

## 12.2 $$L$$-級数の同値性とそこからの帰結

谷山–志村–Weil定理の中心テーマは、**楕円曲線側の Hasse–Weil** $$L$$**-級数**と（本来は複素解析的な性質をもつ）**重み 2 のモジュラー形式** $$f$$ **の** $$L$$**-級数**とを一致させることです。すなわち、
...
2. **Galois 表現**\
   楕円曲線 $$E$$ もモジュラー形式 $$f$$ も、絶対ガロワ群 $$\mathrm{Gal}(\overline{\mathbb{Q}}/\mathbb{Q})$$ への 2 次元表現を自然に定義します（$$E$$ の場合はトーション点への作用、$$f$$ の場合はモジュラーシンボル等による）。谷山–志村–Weil定理は、これら 2 つの表現が同型になることを意味し、リベットの定理で使われる**レベル降下**（level lowering）の議論など、フェルマーの最終定理の証明に不可欠なツールを提供します。
...
これらの例は、どの素数 $$p$$ に対しても**局所的データ**が一致することが谷山–志村–Weil定理の核心であることを示しています。そして一度一致が成り立てば、モジュラー形式に固有の解析的手法が楕円曲線の算術研究にも活用できるようになるのです。
...
* **幾何学 vs. 解析学の視点**\
  楕円曲線は、複素数体上では「格子 $$\Lambda$$ をとって $$\mathbb{C}/\Lambda$$ とみなすトーラス」として理解できるのに対し、モジュラー形式は $$\Gamma_0(N)$$ の変換則を満たす正則関数です。谷山–志村–Weil定理は、これら 2 つの視点が一つの物語に統合されることを示すものです。
* **なぜ重要か**\
...
* **Wiles, A.** “Modular elliptic curves and Fermat’s Last Theorem.” _Annals of Mathematics_, 141(3):443–551, 1995.\
  （半安定な谷山–志村–Weil定理を証明した画期的論文。）

***

**第 12 節 おわり**

この節では、**谷山–志村–Weil定理**（半安定の場合）を導入し、楕円曲線とモジュラー形式の定義や、素数ごとの局所因子の一致に基づく $$L$$-級数の同一視を解説しました。次の節では、**ガロワ表現**や**変形理論**などの技術的背景に踏み込み、こうした同一視がどのように実現され、**フェルマーの最終定理**のような大定理がどのようにして証明されるのかを詳しく見ていきます。
