...
数論、特に **保型形式 (modular forms)** や **ガロア表現 (Galois representations)**、そこから構成される **ヘッケ代数 (Hecke algebras)** の研究において、扱われるある種の環が _Gorenstein_ となることは非常に重要です。ざっくり言えば、これはその環がある種の「自己双対性 (self-duality)」を持ち、環論的・ホモロジー的性質を厳密に制御できることを意味します。
...
* この性質は、$$R$$ が **ヘッケ代数**（ある極大イデアルで局所化したもの）や、剰余ガロア表現の変形を表す **変形環 (deformation ring)** の役割を果たすときに、数論幾何で特に重要になります。
...
### **27.1.3 変形環とヘッケ代数を対応づける理由**

数論においては、次のような状況が典型例です:

* 極大イデアル $$\mathfrak{m}$$ で局所化した **ヘッケ代数** $$\mathbb{T}_\mathfrak{m}$$ が Gorenstein であることがしばしば示される。
...
1. **変形環** $$R_\mathrm{def}$$ から **ヘッケ代数** $$\mathbb{T}_\mathfrak{m}$$ への全射 $$R_\mathrm{def} \twoheadrightarrow \mathbb{T}_\mathfrak{m}$$ を与えること。
...
ある対象、例えばガロア表現 $$ho: \mathcal{G} \to \mathrm{GL}_2(k)$$ を変形するとき、最大イデアルで局所化した変形環の **正接空間 (tangent space)** が、その対象の一階変形の情報を担います。
...
* **変形環** の正接空間（すなわち $$\dim H^1$$ のような次元情報）と、**ヘッケ代数** 側の類似の次元がうまく対応する。
...
局所化したヘッケ代数 $$\mathbb{T}_\mathfrak{m}$$ が Gorenstein であることを証明する標準的なアプローチとしては:
...
バリー・メイザー (Barry Mazur) は、ガロア表現 $$ho$$ に対して局所条件を課すことで $$H^1(\mathcal{G},\mathrm{ad}\,\rho)$$ の形状を制限するという「変形理論的」視点を最初期に発展させました。彼の枠組みは、_普遍変形環 (universal deformation ring)_ を厳密に定義し、これを代数幾何学的対象（形式的冪級数環の商）として扱う流れを確立し、そこにしばしば Gorenstein 性が現れることを示す基礎を築きました。
...
アンドリュー・ワイルズ (Andrew Wiles) がフェルマーの最終定理を証明する際に不可欠だったのが、**普遍変形環** と **（局所化された）ヘッケ代数** の 2 つの環を対応させ、それらが実は同一であることを示すことでした。このとき Gorenstein 性は次のステップで大きな役割を果たします:
